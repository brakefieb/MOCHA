...
Rcpp::List BayeSMART(arma::mat V, arma::mat Y, arma::mat P,  int K, NumericVector e, NumericVector f,
                   ...
                   NumericMatrix sigma_initial, NumericVector alpha, NumericVector omega_initial, arma::mat Z_initial, NumericVector weight, double lambda,
                   int iter, int store_thin);
...
Rcpp::List BayeSMART(arma::mat V, arma::mat Y, arma::mat P,  int K, NumericVector e, NumericVector f, 
                   ...
                   int iter = 20000, int store_thin = 1) {
  ...
  if (store_thin < 1) {
    stop("store_thin must be >= 1");
  }
  int store_iter = (iter + store_thin - 1) / store_thin;
  int store_idx = 0;
  
  int q, qq, qqq, h, count;
  int progress_next = 10;
  
  NumericMatrix mu_store(store_iter, Y_dim*K);
  NumericMatrix mu(K, Y_dim);
  arma::cube sigma(K, Y_dim, Y_dim); // revise
  NumericMatrix sigma_store(store_iter, K*Y_dim*Y_dim); // revise
  NumericMatrix Z_store(store_iter, N);
  NumericVector Z(N);
  NumericMatrix omega_store(store_iter, V_dim*K);
  ...
  for(int t = 0; t < iter; t++)
  {
    ...
    // Store thinned posterior samples
    if (t % store_thin == 0) {
      count = 0;
      for(q = 0; q < K; q++)
      {
        for(qq = 0; qq < Y_dim; qq++)
        {
          mu_store(store_idx, count) = mu(q, qq);
          count++;
        }
      }
      ...
      for(q = 0; q < K; q++)
      {
        ...
        {
          for (qqq = 0; qqq < Y_dim; qqq++){
            sigma_store(store_idx, count) = sigma(q, qq, qqq); // record by rows
            count++;
          }
        }
      }
      ...
      for(q = 0; q < K; q++)
      {
        for(qq = 0; qq < V_dim; qq++)
        {
          omega_store(store_idx, count) = omega(q, qq);
          count++;
        }
      }
      ...
      for(q = 0; q < N; q++)
      {
        Z_store(store_idx, q) = Z(q);
      }
      store_idx++;
    }

    // Monitor progress. This is flushed so Slurm logs update during long runs.
    while (((t + 1) * 100 / iter) >= progress_next && progress_next <= 100)
    {
      Rcout << progress_next << "% has been done" << std::endl;
      progress_next = progress_next + 10;
    }
  }
