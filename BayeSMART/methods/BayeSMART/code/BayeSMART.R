...
# sourceCpp is handled by the external runner script
...
run.BayeSMART <- function(V, Y, G, n_cluster, f_val = 1, w = 1/20,
                          label_switch_refer = 1, lambda = 0,
                          mcmc_iter = 20000, store_thin = 1){
  n_spot <- nrow(Y)
  n_cell_type <- ncol(V)
  n_PC <- ncol(Y)
  mcmc_iter <- as.integer(mcmc_iter)
  store_thin <- as.integer(store_thin)
  if (mcmc_iter < 1) {
    stop("mcmc_iter must be >= 1")
  }
  if (store_thin < 1) {
  ...
  # run iIMPACT
  cat("Running BayeSMART MCMC with iter =", mcmc_iter,
      "and store_thin =", store_thin, "\n")
  flush.console()
  result <- BayeSMART(
    V, Y, G, n_cluster, e, f, eta, tau, alpha_gamma, beta_gamma,
    mu_initial, sigma_initial, alpha, omega_initial, Z_initial,
    rep(w, n_spot), lambda = lambda, iter = mcmc_iter,
