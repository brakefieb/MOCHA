# BayeSMART: MOCHA reproducibility guide
...
"${PY_ENV_ROOT}/bin/python" code_Xin/main.py --method BayeSMART --cohort DLPFC_10x
```

MOCHA integration files:

- Runner: `code_Xin/runners/run_bayesmart.py and run_bayesmart.R`
- Method defaults: `configs/methods/bayesmart.yaml`
- Cohort overrides: `configs/experiments/BayeSMART/`
- Environment setup/check: `envs/setup_bayesmart_hpc.sh`, `envs/check_bayesmart_hpc.sh`
- HPC job: `scripts/run_bayesmart.slurm`
- Detailed guide: [docs/bayesmart_hpc.md](../../docs/bayesmart_hpc.md)

Input data is intentionally not included. Configure local data paths in `configs/cohorts/*.yaml`. Outputs are written under `results/BayeSMART/<cohort>/` and are also excluded from Git.
