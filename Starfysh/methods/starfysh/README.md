# Starfysh: MOCHA reproducibility guide
...
bash envs/setup_starfysh_hpc.sh "${PY_ENV_ROOT}" "${PROJECT_ROOT}"
bash envs/check_starfysh_hpc.sh "${PY_ENV_ROOT}" "${PROJECT_ROOT}"
"${PY_ENV_ROOT}/bin/python" code_Xin/main.py --method Starfysh --cohort DLPFC_10x
```

MOCHA integration files:

- Runner: `code_Xin/runners/run_starfysh.py`
- Method defaults: `configs/methods/starfysh.yaml`
- Cohort overrides: `configs/experiments/Starfysh/`
- Environment setup/check: `envs/setup_starfysh_hpc.sh`, `envs/check_starfysh_hpc.sh`
- HPC job: `scripts/run_starfysh.slurm`
- Detailed guide: [docs/starfysh_hpc.md](../../docs/starfysh_hpc.md)

Input data is intentionally not included. Configure local data paths in `configs/cohorts/*.yaml`. Outputs are written under `results/Starfysh/<cohort>/` and are also excluded from Git.
