# stGCL: MOCHA reproducibility guide
...
bash envs/setup_stgcl_hpc.sh "${PY_ENV_ROOT}" "${PROJECT_ROOT}"
bash envs/check_stgcl_hpc.sh "${PY_ENV_ROOT}" "${PROJECT_ROOT}"
"${PY_ENV_ROOT}/bin/python" code_Xin/main.py --method stGCL --cohort DLPFC_10x
```

MOCHA integration files:

- Runner: `code_Xin/runners/run_stgcl.py`
- Method defaults: `configs/methods/stgcl.yaml`
- Cohort overrides: `configs/experiments/stGCL/`
- Environment setup/check: `envs/setup_stgcl_hpc.sh`, `envs/check_stgcl_hpc.sh`
- HPC job: `scripts/run_stgcl.slurm`
- Detailed guide: [docs/stgcl_hpc.md](../../docs/stgcl_hpc.md)

Input data is intentionally not included. Configure local data paths in `configs/cohorts/*.yaml`. Outputs are written under `results/stGCL/<cohort>/` and are also excluded from Git.
