...
def trainer(adata, data_name, save_path, domains=None,
            ...
            graph_dist_type="BallTree", graph_k=12, rad_cutoff=150):
    ...
    if domains is not None:
        n_batch_domains = int(len(np.unique(domains)))
        domains = torch.from_numpy(domains).to(device)
    else:
        n_batch_domains = None
        domains = domains

    graph_dict = graph_utils.graph(
        adata.obsm['spatial'],
        distType=graph_dist_type,
        k=graph_k,
    ...
    if use_model:
        ...
        if domains is not None:
            model = AdversarialNetwork(model=model, n_domains=n_batch_domains)
        else:
            model = model
        ...
        with torch.no_grad():
            ...
            if domains is None:
                ...
                pre_z, _, _, _, _, _, _, _ = model(data, adj)
    ...
    with torch.no_grad():
        ...
        if domains is None:
            ...
            z, mu, logvar, de_feat, out_q, feat_x, gnn_z, _ = model(data, adj)
