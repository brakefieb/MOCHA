...
def cal_weight_matrix(
    ...
    if platform in ["Visium", "ST", "slideseqv2", "stereoseq"]:
        ...
        gene_correlation = 1 - pairwise_distances(to_dense_array(gene_counts), metric=gb_dist_type)
    del gene_counts
...
def find_adjacent_spot(
    ...
    if use_data == "raw":
        gene_matrix = to_dense_array(adata.X)
    else:
...
def enhance_gene_data(
    ...
    enhance_gene_matrix = to_dense_array(adata.X) + adjacent_weight * adjacent_gene_matrix
