# ResST: MOCHA reproducibility guide
...
bash envs/setup_resst_hpc.sh "${PY_ENV_ROOT}" "${PROJECT_ROOT}"
bash envs/check_resst_hpc.sh "${PY_ENV_ROOT}" "${PROJECT_ROOT}"
"${PY_ENV_ROOT}/bin/python" code_Xin/main.py --method ResST --cohort DLPFC_10x
```

MOCHA integration files:

- Runner: `code_Xin/runners/run_resst.py`
- Method defaults: `configs/methods/resst.yaml`
- Cohort overrides: `configs/experiments/ResST/`
- Environment setup/check: `envs/setup_resst_hpc.sh`, `envs/check_resst_hpc.sh`
- HPC job: `scripts/run_resst.slurm and scripts/run_resst_gpu.slurm`
- Detailed guide: [docs/resst_hpc.md](../../docs/resst_hpc.md)

Input data is intentionally not included. Configure local data paths in `configs/cohorts/*.yaml`. Outputs are written under `results/ResST/<cohort>/` and are also excluded from Git.
