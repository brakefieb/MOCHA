# DeepST: MOCHA reproducibility guide
...
bash envs/setup_deepst_hpc.sh "${PY_ENV_ROOT}" "${PROJECT_ROOT}"
bash envs/check_deepst_hpc.sh "${PY_ENV_ROOT}" "${PROJECT_ROOT}"
"${PY_ENV_ROOT}/bin/python" code_Xin/main.py --method DeepST --cohort DLPFC_10x
```

MOCHA integration files:

- Runner: `code_Xin/runners/run_deepst.py`
- Method defaults: `configs/methods/deepst.yaml`
- Cohort overrides: `configs/experiments/DeepST/`
- Environment setup/check: `envs/setup_deepst_hpc.sh`, `envs/check_deepst_hpc.sh`
- HPC job: `scripts/run_deepst.slurm and scripts/run_deepst_gpu.slurm`
- Detailed guide: [docs/deepst_hpc.md](../../docs/deepst_hpc.md)

Input data is intentionally not included. Configure local data paths in `configs/cohorts/*.yaml`. Outputs are written under `results/DeepST/<cohort>/` and are also excluded from Git.
